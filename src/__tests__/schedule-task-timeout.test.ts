...
//
// v1.34.1 hybrid merge (kanban e442c5c0): origin/main independently added its
// own 'lost' delivery tracking (sawTurn, see decideTaskTimeout's 2026-08-23
// incident doc) for a DIFFERENT bug -- a session wedged at 100% context
// accepts keystrokes and reads idle forever without ever running the task.
// isTaskCompletionEvidence gained a second parameter (sawTurn) so that same
// "idle but never actually ran" state cannot ALSO be misread as reauth-healer
// liveness proof -- the two fixes address different symptoms of the same
// underlying gap (idle alone is not proof of anything) and had to be merged
// together, not just concatenated.
