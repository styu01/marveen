...
describe('schedule-runner treats a transiently-down main session as "starting", not "missing"', () => {
  ...
  it('the pending-retry loop only permanently deletes on "fired", never "missing" or "starting" (v1.34.1 hybrid merge: origin/main independently went further than the original fix here -- "missing" no longer deletes AT ALL, not just for the main agent -- see docs/marveen-v1.34.1-hybrid-merge-dev-spec.md)', () => {
